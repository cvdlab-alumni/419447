...
p0 = [[2.7, 4.03, 0], [4.54, 4.12, 0]]
s0 = BEZIER(S1)(p0)
p1 = [[2.7, 4.03, 0], [4.58, 4.23, 0.05]]
s1 = BEZIER(S1)(p1)

#VIEW(STRUCT([MAP(s0)(GRID([10,10])),MAP(s1)(GRID([10,10])),MAP(s2)(GRID([10,10])),MAP(s3)(GRID([10,10]))]))

parte_inferiore = COLOR(YELLOW)(MAP(BEZIER(S2)([s0,s1]))(GRID([20,20])))

p2 = [[2.8, 4.71,0.2], [2.71, 4.54,0.2],  [4.21, 4.79,0.2],  [4.69, 4.75,0.2]]
s2 = BEZIER(S1)(p2)
p3 = [[2.1, 4.83,-0.1], [2.89, 4.67,-0.1], [3.33, 4.81,-0.1],  [4.66, 4.89, -0.1]]
s3 = BEZIER(S1)(p3)
p4 = [[2.23, 4.98, -0.24], [2.89, 4.78, -0.24], [3.44, 4.92, -0.24],  [4.64, 5, -0.24],]
s4 = BEZIER(S1)(p4)

parte_centrale = COLOR(YELLOW)(MAP(BEZIER(S2)([s1,s2,s3,s4]))(GRID([20,20])))

p5 = [[4.48, 4.98, -0.24], [4.64, 5, -0.24]]
s5 = BEZIER(S1)(p5)
p6 = [[4.43, 5.42, -0.8], [4.58, 5.42, -0.8]]
s6 = BEZIER(S1)(p6)

finestrino_lato = COLOR(BLACK)(MAP(BEZIER(S2)([s5,s6]))(GRID([20,20])))

p7 = [[4.48, 4.97, -0.24], [4.01, 4.97, -0.24], [3.43, 4.82, -0.24], [2.88, 4.88, -0.24]]
s7 = BEZIER(S1)(p7)
p8 = [[4.44, 5.36, -0.74], [3.76, 5.41, -0.78], [3.45, 5.26, -0.6], [2.89, 5.11, -0.55]]
s8 = BEZIER(S1)(p8)

finestrino = COLOR(CYAN)(MAP(BEZIER(S2)([s7,s8]))(GRID([20,20])))

p9 = [[2.41, 4.93, -0.24], [2.67, 4.85, -0.24], [2.68, 4.85, -0.24], [2.89, 4.87, -0.24]]
s9 = BEZIER(S1)(p9)
p10 =[[2.41, 4.93, -0.24], [2.58, 5.05, -0.5], [2.76, 5.1, -0.6], [2.89, 5.15, -0.6]]
s10 = BEZIER(S1)(p10)

specchietto_base = COLOR(BLACK)(MAP(BEZIER(S2)([s9,s10]))(GRID([20,20])))

s11 = BEZIER(S1)(p8)
s12 = BEZIER(S1)([[4.44, 5.42, -0.8], [3.68, 5.41, -0.78], [3.33, 5.27, -0.78], [2.89, 5.15, -0.6]])

finestrino_sopra = COLOR(BLACK)(MAP(BEZIER(S2)([s11,s12]))(GRID([20,20])))

p13 = [[2.41, 4.93, -0.24], [2.78, 5.17, -0.8], [3.64, 5.43, -0.8], [4.58, 5.42, -0.8]]
s13 = BEZIER(S1)(p13)
p14 = [[2.23, 4.98, -0.24], [3.05, 5.35, -0.9], [3.9, 5.54, -0.9], [4.58, 5.47, -0.9]]
s14 = BEZIER(S1)(p14)

parte_superiore = COLOR(YELLOW)(MAP(BEZIER(S2)([s13,s14]))(GRID([20,20])))

portiera = STRUCT([parte_centrale, parte_inferiore, finestrino_lato, finestrino, specchietto_base, finestrino_sopra, parte_superiore])

portiera = R([2,3])(PI/2)(T([1,2])([-2.23,-4.03])(portiera))

portiera_in_posizione = T([1,3])([2.2, 0.3])(portiera) 

whole = STRUCT([center_mode, wheels, steering_wheel, portiera_in_posizione])
VIEW(whole)