...
var road = function(coordinate, rows, cols) {
	var p = [];

	for (var row=0; row<=rows; row++) {
		for (var col=0; col<=cols; col++) {
			p.push(get_point(points, [coordinate[0]+row, coordinate[1]+col]))
		}	
	}
	
	var cells = coordinates_to_cells(rows+1, cols+1);
	return COLOR(GREY)(T([2])([0.01])(SIMPLICIAL_COMPLEX(p)(cells)));
}


var district = function (centro) {
	var qc = bunch_of_houses(centro);
	var sr = road(south(west(centro)), 1, 3);
	var er = road(north(north(east(centro))), 3, 1);
	var nr = road(north(west(north(west(centro)))),1, 3);
	var wr = road(north(west(west(centro))),3, 1);
	return STRUCT([sr, er, nr, wr, qc]);
}


var out = STRUCT([lake, road([10,18],1,32),
				  forest([25,50],7,40),forest([30,50],7,80),forest([32,8],5,60),
				  district([10,7]),district([10,10]),district([10,13]),district([10,16]),district([12,50]),district([12,53]),district([15,53]),district([9,50]),district([9,53]),
				  terrain_model]);
